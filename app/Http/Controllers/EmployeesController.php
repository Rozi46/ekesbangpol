...
{ 
    // isi select
        public function listopjabatan(Request $request)
        {
            try { 
                ...
                $response = app('App\Services\ApiEmployees')->listopjabatan($request);            
                $results = is_array($response) ? $response : $response->getData(true); 

                return $results;
                
            } catch (Throwable $e) {
                Log::error('datalistpegawai Error: ' . $e->getMessage(), ['user' => $request->session()->get('admin_login_kesbangpol')]);
        ...
        public function listoppangkat(Request $request)
        {
            try { 
                ...
                $response = app('App\Services\ApiEmployees')->listoppangkat($request);            
                $results = is_array($response) ? $response : $response->getData(true); 
    ...
    public function listpegawai(Request $request)
    {
        try { 
            ...
            $menu = 'masterdata';
            $action = 'listpegawai';
            $viewpath = 'admin.AdminOne.masterdata.listdata.pegawai';
            ...
            Log::error('listpegawai Error: ' . $e->getMessage(), ['user' => $request->session()->get('admin_login_kesbangpol')]);
            return redirect('/admin/logout')->with('error', 'Terjadi kesalahan sistem.');
        }        
    }

    public function datalistpegawai(Request $request)
    {
        try { 
            if (!session()->has('key_token_kesbangpol') || !session()->has('admin_login_kesbangpol') || 
                empty(session('key_token_kesbangpol')) || empty(session('admin_login_kesbangpol'))) {
    ...
    public function savepegawai(Request $request)
    {
        try {
            ...
            $response = app('App\Services\ApiEmployees')->savepegawai($request);
            $results  = is_array($response) ? $response : $response->getData(true);
            ...
            Log::error('savepegawai Error: ' . $e->getMessage(), ['user' => $request->session()->get('admin_login_kesbangpol')]);
    ...
    public function viewpegawai(Request $request)
    {
        try {
            ...
            $response = app('App\Services\ApiEmployees')->viewpegawai($request);
            $results  = is_array($response) ? $response : $response->getData(true);
            ...
            $data = $results['results']['pegawai'] ?? $results['results'] ?? [];

            return response()->json(['status_message' => 'success','note' => 'Data berhasil dimuat','data' => $data], 201);

        } catch (Throwable $e) {
            Log::error('detailpegawai Error: ' . $e->getMessage(), ['user' => $request->session()->get('admin_login_kesbangpol')]);
            return response()->json(['status_message' => 'error', 'note' => 'Terjadi kesalahan sistem: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function updatepegawai(Request $request)
    {
        try {
            ...
            $response = app('App\Services\ApiEmployees')->editpegawai($request);
            $results  = is_array($response) ? $response : $response->getData(true);
            ...
            Log::error('updatepegawai Error: ' . $e->getMessage(), ['user' => $request->session()->get('admin_login_kesbangpol')]);
            return response()->json(['status_message' => 'error', 'note' => 'Terjadi kesalahan sistem: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function statuspegawai(Request $request)
    {
        try { 
            ...
            $response = app('App\Services\ApiEmployees')->upstatuspegawai($request);
            $results = is_array($response) ? $response : $response->getData(true);

            return response()->json($results);
            
        } catch (Throwable $e) {
            Log::error('statuspegawai Error: ' . $e->getMessage(), ['user' => $request->session()->get('admin_login_kesbangpol')]);
            return redirect('/admin/logout')->with('error', 'Terjadi kesalahan sistem.');
        }
    }
    ...
    public function deletepegawai(Request $request)
    {
        try { 
            ...
            $response = app('App\Services\ApiEmployees')->deletepegawai($request);
            $results = is_array($response) ? $response : $response->getData(true);

            return response()->json($results);
            
        } catch (Throwable $e) {
            Log::error('deletepegawai Error: ' . $e->getMessage(), ['user' => $request->session()->get('admin_login_kesbangpol')]);
            return redirect('/admin/logout')->with('error', 'Terjadi kesalahan sistem.');
        }
    }
    ...
    public function exportpegawai(Request $request)
    {
        try { 
            ...
            $fileName = "Data-pegawai-".date('Y-m-d-His').".xls" ;
            Excel::store(new Pegawai($request),'exports/' . $fileName,'public');

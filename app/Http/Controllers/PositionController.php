...
{ 
    public function listjabatan(Request $request)
    {
        try { 
            ...
            $request['url_active'] = 'listjabatan';

            $menu = 'masterdata';
            $action = 'listjabatan';
            $viewpath = 'admin.AdminOne.masterdata.listdata.jabatan';
            ...
            Log::error('listjabatan Error: ' . $e->getMessage(), ['user' => $request->session()->get('admin_login_kesbangpol')]);
            return redirect('/admin/logout')->with('error', 'Terjadi kesalahan sistem.');
        }        
    }

    public function datalistjabatan(Request $request)
    {
        try { 
            if (!session()->has('key_token_kesbangpol') || !session()->has('admin_login_kesbangpol') || 
                empty(session('key_token_kesbangpol')) || empty(session('admin_login_kesbangpol'))) {
            ...
            $response = app('App\Services\ApiPosition')->listjabatan($request);            
            $results = is_array($response) ? $response : $response->getData(true); 

            return $results;
            
        } catch (Throwable $e) {
            Log::error('datalistjabatan Error: ' . $e->getMessage(), ['user' => $request->session()->get('admin_login_kesbangpol')]);
            return redirect('/admin/logout')->with('error', 'Terjadi kesalahan sistem.');
        }        
    }
    ...
    public function savejabatan(Request $request)
    {
        try {
            ...
            $response = app('App\Services\ApiPosition')->savejabatan($request);
            $results  = is_array($response) ? $response : $response->getData(true);
            ...
            Log::error('savejabatan Error: ' . $e->getMessage(), ['user' => $request->session()->get('admin_login_kesbangpol')]);
    ...
    public function viewjabatan(Request $request)
    {
        try {
            ...
            $response = app('App\Services\ApiPosition')->viewjabatan($request);
            $results  = is_array($response) ? $response : $response->getData(true);
            ...
            $data = $results['results']['jabatan'] ?? $results['results'] ?? [];

            return response()->json(['status_message' => 'success','note' => 'Data berhasil dimuat','data' => $data], 201);

        } catch (Throwable $e) {
            Log::error('detailjabatan Error: ' . $e->getMessage(), ['user' => $request->session()->get('admin_login_kesbangpol')]);
            return response()->json(['status_message' => 'error', 'note' => 'Terjadi kesalahan sistem: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function updatejabatan(Request $request)
    {
        try {
            ...
            $response = app('App\Services\ApiPosition')->editjabatan($request);
            $results  = is_array($response) ? $response : $response->getData(true);
            ...
            Log::error('updatejabatan Error: ' . $e->getMessage(), ['user' => $request->session()->get('admin_login_kesbangpol')]);
            return response()->json(['status_message' => 'error', 'note' => 'Terjadi kesalahan sistem: ' . $e->getMessage()], 500);
        }
    }

    public function statusjabatan(Request $request)
    {
        try { 
            if (!session()->has('key_token_kesbangpol') || !session()->has('admin_login_kesbangpol') || 
                empty(session('key_token_kesbangpol')) || empty(session('admin_login_kesbangpol'))) {
            ...
            $response = app('App\Services\ApiPosition')->upstatusjabatan($request);
            $results = is_array($response) ? $response : $response->getData(true);

            return response()->json($results);
            
        } catch (Throwable $e) {
            Log::error('statusjabatan Error: ' . $e->getMessage(), ['user' => $request->session()->get('admin_login_kesbangpol')]);
            return redirect('/admin/logout')->with('error', 'Terjadi kesalahan sistem.');
        }
    }
    ...
    public function deletejabatan(Request $request)
    {
        try { 
            ...
            $response = app('App\Services\ApiPosition')->deletejabatan($request);
            $results = is_array($response) ? $response : $response->getData(true);

            return response()->json($results);
            
        } catch (Throwable $e) {
            Log::error('deletejabatan Error: ' . $e->getMessage(), ['user' => $request->session()->get('admin_login_kesbangpol')]);
            return redirect('/admin/logout')->with('error', 'Terjadi kesalahan sistem.');
        }
    }
    ...
    public function exportjabatan(Request $request)
    {
        try { 
            ...
            $fileName = "Data-Jabatan-".date('Y-m-d-His').".xls" ;
            Excel::store(new Jabatan($request),'exports/' . $fileName,'public');

...
function openSidebar() {
    ...
    $('.bg_logo_sidebar img').attr('src', "/image/setting/logo.png");
...
function closeSidebar() {
	...
	let imageUrl = foto ? "/image/setting/" + foto : "/image/setting/icon.png";
...
function openSearchAjax() {
    if($('input[name="key-search-ajax"]').is(":hidden")){
        $('input[name="key-search-ajax"]').show().css({"width": "200px","transition":"0.5s"}).focus();
		$('[btn="openSearchajax"]').removeClass('btn-default').addClass('btn-danger');
		$('[btn="openSearchajax"] i').removeClass('fa fa-search').addClass('fa fa-times');
    }else{
		$('input[name="key-search-ajax"]').css({"width": "0px","transition":"0.5s"});
		setTimeout(function(){ $('input[name="key-search-ajax"]').hide(); }, 400);
		$('[btn="openSearchajax"]').removeClass('btn-danger').addClass('btn-default');
		$('[btn="openSearchajax"] i').removeClass('fa fa-times').addClass('fa fa-search');
    }
}
...
// export data
	// function exportdata(namedata, btn = null) {
	...
	// 	const searchdate = (startVal && endVal)
	// 		? `${formatDate(startVal)}sd${formatDate(endVal)}`
	// 		: '';

	// 	$.ajax({
	// 		url: `prepare-export${namedata}`,
	// 		type: 'POST',
	// 		dataType: 'json',
	// 		data: {
	// 			page: 1,
	// 			vd: $('#countvd').val(),
	// 			keysearch: $('input[name="key-search"]').val(),
	// 			searchdate
	// 		},
	// 		success: function (res) {
	// 			if (res.success) {
	// 				window.location.href = res.download_url;
	// 				SystemToast('success', 'Export berhasil, file sedang diunduh');
	// 			} else {
	// 				SystemToast('danger', res.message || 'Export gagal');
	// 			}
	// 		},
	// 		error: function (xhr) {
	// 			SystemToast(
	// 				'danger',
	// 				xhr.responseJSON?.message || 'Terjadi kesalahan saat export'
	// 			);
	// 		},
	// 		complete: function () {
	...
	// async function exportdata(namedata, btn = null) {
	// 	try {
	// 		if (btn) {
	// 			$(btn)
	// 				.prop('disabled', true)
	// 				.html('<i class="fa fa-spinner fa-spin"></i> Exporting...');
	// 		}
	...
	// 			url: `/admin/prepare-export${namedata}`,
	// 			type: 'POST',
	// 			data: payload
	// 		});

	// 		if (!response.success) {
	// 			throw new Error(response.message || 'Export gagal');
	// 		}

	// 		SystemToast('success', 'File siap, download dimulai...');

	// 		window.location.href = response.download_url;

	// 	} catch (error) {
	// 		SystemToast('danger',error.responseJSON?.message || error.message || 'Export gagal');
	// 	} finally {
	// 		if (btn) {
	// 			$(btn)
	// 				.prop('disabled', false)
	// 				.html('<i class="fa fa-download"></i> Export Data');
	// 		}
	// 	}
	// }
	...
	async function exportdata({
		...
		try {
			...
			SystemToast(
				'danger',
				error.responseJSON?.message || error.message || 'Export gagal'
			);
		} finally {
			if (btn) {
				$(btn)
...
function expandSidebar() {
    ...
    $('.bg_logo_sidebar').removeClass('bg_logo_sidebar_min')
                        .find('img').attr('src', "/image/setting/logo.png");
    $('.list_sidebar').removeClass('list_sidebar_min');
    $('.list_sidebar .fa-users').css("margin-left", "0px");
}

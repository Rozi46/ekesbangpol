...
<style>
    ...
    .upload-placeholder { text-align: center; padding: 10px 0; }
    .upload-placeholder i { color: #4f7cff; margin-bottom: 10px; display: block; }
</style>
                                        ...
                                        <div class="field-wrapper" id="wrap_filepath">
                                            <div class="modern-upload-card" id="dropzonePhoto">
                                                ...
                                                <div id="uploadPlaceholder" class="upload-placeholder">
                                                    ...
                                                    <h5 style="margin-bottom:4px;">Upload File Arsip</h5>
...
$(function () {
    ...
    /* =========================================================
     * HELPER — ambil nama file dari path apapun (Unix / Windows)
     * ========================================================= */
    ...
    /* =========================================================
     * HELPER — icon & warna berdasarkan ekstensi file
     * ========================================================= */
    function getFileIconColor(filePath) {
        ...
        return map[ext] || { icon: 'fa-file-alt', color: '#4f7cff' };
    }

    /* =========================================================
     * FILE PREVIEW — state
     * ========================================================= */
    let savedFileOnEdit = false; // flag: ada file tersimpan di server (mode edit)
    ...
    function previewSavedFile(filePath) {
        ...
        $('#previewSize').text('File tersimpan');
        $('#uploadProgressWrap').hide();
        $('#uploadPlaceholder').hide();
    ...
    function hasFile() {
        const hasNew = $('#field_filepath')[0].files && $('#field_filepath')[0].files.length > 0;
        return hasNew || savedFileOnEdit;
    }

    /* =========================================================
     * DROPZONE — event (satu kali saja)
     * ========================================================= */
    const dropzone  = $('#dropzonePhoto');
    const inputFile = $('#field_filepath');

    // Klik area dropzone → buka file picker
    // (kecuali klik pada tombol #btnChooseFile atau #btnRemoveFile)
    dropzone.on('click', function (e) {
        if ($(e.target).closest('#btnChooseFile, #btnRemoveFile').length) return;
        inputFile.trigger('click');
    });

    // Cegah input file mem-bubble klik ke dropzone
    ...
    /* =========================================================
     * SELECT2 — inisialisasi SATU KALI
     * ========================================================= */
        ...
        $.ajax({
            ...
            success: function (res) {
                const found = (res.results || []).find(function (item) {
                    return item.code_data == selected;
                });
                if (found) {
                    if (!$('#field_codekategori option[value="' + found.code_data + '"]').length) {
                        $('#field_codekategori').append(new Option(found.nama_kategori, found.code_data, true, true));
                    } else {
                        $('#field_codekategori').val(found.code_data);
                    }
                    $('#field_codekategori').trigger('change');
                }
            }
        });
    }
    ...
    function renderHeader() {
        ...
        switch (state.mode) {
            case 'list':
                if (action.new)    buttons += '<button type="button" class="btn btn-primary" id="btnTambah"><i class="fa fa-plus"></i> Tambah Data</button> ';
                if (action.export) buttons += '<button type="button" class="btn btn-info" onclick="exportdata({url:\'/admin/exportarsip\', btn:this})"><i class="fa fa-download"></i> Export Data</button>';
                break;
    ...
    function setFieldError(wrapperId, errId, message) {
        ...
        $err.css('color', '#e0294a').find('i, span').css('color', '#e0294a');
        $err.show();
    }
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (target === 'view') {
                    ...
                    $('#view_codekategori').text(d.code_kategori        ?? '-');
    ...
    /* =========================================================
     * RENDER FILE — VIEW panel (card style seperti Google Drive)
     * ========================================================= */
    function renderFileView(filePath, selector) {
        ...
        const linkHtml        = isLocalPath
            ? '<span class="btn btn-sm btn-secondary disabled"><i class="fa fa-exclamation-circle"></i> Path tidak valid</span>'
            : '<a href="/storage/' + filePath + '" target="_blank" class="btn btn-primary btn-sm"><i class="fa fa-eye"></i> Lihat</a>';

        // $(selector).html(
        //     '<div style="display:inline-flex; align-items:center; gap:12px;' +
        //                  'background:#f8f9fa; border:1px solid #dee2e6;' +
        //                  'border-radius:10px; padding:10px 14px; max-width:420px; width:100%;">' +
        //         '<div style="width:48px; height:48px; min-width:48px; background:#fff;' +
        //                     'border-radius:10px; border:1px solid #e0e4f0;' +
        //                     'display:flex; align-items:center; justify-content:center;">' +
        //             '<i class="fa ' + icon + ' fa-2x" style="color:' + color + ';"></i>' +
        //         '</div>' +
        //         '<div style="flex:1; min-width:0;">' +
        //             '<div style="font-weight:600; font-size:14px; white-space:nowrap;' +
        //                         'overflow:hidden; text-overflow:ellipsis;" title="' + filename + '">' + filename + '</div>' +
        //             '<small style="color:#6b7280;">Dokumen Arsip</small>' +
        //         '</div>' +
        //         linkHtml +
        //     '</div>'
        // );
        ...
        $(selector).html(`
            <div class="file-card">
                ...
                <div class="file-info">
                    ...
                    <div class="file-path" title="${filename}">
                        ${filename}
                    </div>
                </div>
    ...
    /* =========================================================
     * RENDER FILE — LIST tabel (icon + nama singkat)
     * ========================================================= */
    function shortFilename(filename, maxLength = 6) {
    ...
    function renderFileList(filePath) {
        ...
        const { icon, color } = getFileIconColor(filename);
        const isLocalPath     = /^[a-zA-Z]:[\\\/]/.test(filePath);
        const href            = isLocalPath ? '#' : '/storage/' + filePath;
        const target          = isLocalPath ? '' : 'target="_blank"';  
        
        return `
        <a href="${href}" ${target} style="display:inline-flex; align-items:center; gap:6px; text-decoration:none;">
            <i class="fa ${icon}" style="color:${color};"></i>
        ...
        //                   'max-width:160px; text-decoration:none; color:inherit;"' +
        //           'title="' + filename + '">' +
        //           '<i class="fa ' + icon + '" style="color:' + color + '; font-size:14px; flex-shrink:0;  margin-right:50px;"></i>' +
        //           '<span style="font-size:12px; white-space:nowrap; overflow:hidden;' +
        //                        'text-overflow:ellipsis; max-width:110px;">' + filename + '</span>' +
        //        '</a>';
    }
    ...
    $('#btnSaveForm').on('click', function () {
        ...
        if (!hasFile())       { setFieldError('wrap_filepath',      'err_filepath',      'File arsip wajib diisi.');        hasError = true; }
        ...
        $.ajax({
            ...
            error: function (xhr) {
                SystemToast('danger', (xhr.responseJSON && xhr.responseJSON.note) ? xhr.responseJSON.note : 'Gagal menyimpan data');
            },
            complete: function () {
    ...
    function renderTable(res) {
        ...
        res.data.forEach(function (item, index) {
            ...
            //                 '<div class="dropdown-menu">' +
            //                     '<h5 class="dropdown-header">Pengaturan Data</h5>' +
            //                     '<a class="dropdown-item btn-view" style="cursor:pointer;" data-code="' + item.code_data + '">Lihat Data</a>' +
            //                     '<a class="dropdown-item btn-edit ' + editCls + '" style="cursor:pointer;" data-code="' + item.code_data + '" ' + editAttr + '>Ubah Data</a>' +
            //                     '<a class="dropdown-item delete-data ' + deleteCls + '" style="cursor:pointer;" data-code="' + item.code_data + '" data-name="' + (item.judul || '') + '" ' + deleteAttr + '>Hapus Data</a>' +
            //                 '</div>' +
            //             '</div>' +
            //         '</td>' +
            //     '</tr>';
                    ...
                    <td class="colright text-center">
                        <div class="dropdown dropleft">
                            ...
                            <div class="dropdown-menu">
                                ...
                                <a
                                    class="dropdown-item btn-edit ${editCls}"
                                    style="cursor:pointer;"
                                    data-code="${item.code_data}"
                                    ${editAttr}>
                                    Ubah Data
                                ...
                                <a
                                    class="dropdown-item delete-data ${deleteCls}"
                                    style="cursor:pointer;"
                                    data-code="${item.code_data}"
        ...
        $(document)
            ...
            .on('click',  '[btn-action="action-confirmasi"]', function () {
                ...
                $.ajax({
                    ...
                    error: function (xhr) {
                        SystemToast('danger', (xhr.responseJSON && xhr.responseJSON.note) ? xhr.responseJSON.note : 'Data gagal dihapus');
                    },

    ...
    <div class="container-fluid text-left">
        <div class="row">
            ...
            <div class="col-md-12 bg_page_main hd" line="hd_action">
                <div class="col-md-12 hd_page_main" id="pageTitle">Data Pangkat</div>
                            ...
                            <table class="table_view table-striped table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center sortable" data-sort="jenis_cuti">Pangkat <i class="fa fa-sort"></i></th>
                                        <th class="text-center sortable" data-sort="jenis_cuti">Golongan <i class="fa fa-sort"></i></th>
                                        <th class="text-center sortable" data-sort="jenis_cuti">Ruang <i class="fa fa-sort"></i></th>
                                        <th width="100" class="text-center">Status</th>
                                        <th width="100" class="text-center"><i class="head fa fa-cog"></i></th>
                                    </tr>
                                </thead>
                                <tbody id="PangkatTableBody">
                                    <tr>
            ...
            <div class="col-md-12 bg_page_main form_action" id="panel-form" style="display:none;" line="form_action">
                ...
                <div class="col-md-12 bg_form_page">
                    <div class="form_input text-left">
                        ...
                        <input type="text" id="field_pangkat" class="form-control" placeholder="Masukkan pangkat..." maxlength="100"> 
                        <div class="invalid-feedback" id="err_pangkat"></div>
                    </div>
                </div>
                <div class="col-md-12 bg_form_page">
                    <div class="form_input text-left">
                        <div class="tag_title">Golongan <span>*</span></div>
                        <input type="text" id="field_golongan" class="form-control" placeholder="Masukkan golongan..." maxlength="100"> 
                        <div class="invalid-feedback" id="err_golongan"></div>
                    </div>
                </div>
                <div class="col-md-12 bg_form_page">
                    <div class="form_input text-left">
                        <div class="tag_title">Ruang <span>*</span></div>
                        <input type="text" id="field_ruang" class="form-control" placeholder="Masukkan ruang..." maxlength="100"> 
                        <div class="invalid-feedback" id="err_ruang"></div>
                    </div>
                </div>
                <div class="col-md-12 bg_form_page">
                    <div class="form_input text-left">
            ...
            <div class="col-md-12 bg_page_main form_action" id="panel-view" style="display:none;" line="form_action">
                ...
                <div class="col-md-12 bg_form_page">
                    <div class="form_input text-left">
                        @if(($level_user['editpangkat'] ?? 'No') === 'Yes')
...
<script>
    $(function () {
        const routes = {
            list:   "{{ url('/admin/datalistpangkat') }}",
            store:  "{{ url('/admin/savepangkat') }}",
            detail: "{{ url('/admin/viewpangkat') }}",
            update: "{{ url('/admin/updatepangkat') }}",
            status: "{{ url('/admin/statuspangkat') }}",
            delete: "{{ url('/admin/deletepangkat') }}"
        };

        const action = {
            new:    {{ (($level_user['newpangkat'] ?? 'No') === 'Yes') ? 'true' : 'false' }},
            edit:   {{ (($level_user['editpangkat'] ?? 'No') === 'Yes') ? 'true' : 'false' }},
            delete: {{ (($level_user['deletepangkat'] ?? 'No') === 'Yes') ? 'true' : 'false' }},
            export: {{ (($level_user['exportpangkat'] ?? 'No') === 'Yes') ? 'true' : 'false' }}
        };
        ...
        function renderHeader() {
            ...
            switch (state.mode) {
                case 'list':
                    ...
                    if (action.export) {
                        buttons += `<button type="button" class="btn btn-info" onclick="exportdata({url:'/admin/exportpangkat',btn:this})"><i class="fa fa-download"></i> Export Data</button>`;
                    }
                    break;
        ...
        function resetForm() {
            $('#field_code_data').val('');
            $('#field_pangkat').val('').removeClass('is-invalid');
            $('#field_golongan').val('').removeClass('is-invalid');
            $('#field_ruang').val('').removeClass('is-invalid');
        ...
        function loadDetail(code, target) {
            ...
            if (target === 'view') {
                $('#view_code_data, #view_pangkat, #view_golongan, #view_ruang, #view_status, #view_created_at, #view_updated_at').html(loading);
            }
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if (target === 'view') {
                        ...
                        $('#view_pangkat').text(d.pangkat ?? '-');
                        $('#view_golongan').text(d.golongan ?? '-');
                        $('#view_ruang').text(d.ruang ?? '-');
                    ...
                    if (target === 'edit') {
                        ...
                        $('#field_pangkat').val(d.pangkat ?? '');
                        $('#field_golongan').val(d.golongan ?? '');
                        $('#field_ruang').val(d.ruang ?? '');
        ...
        $('#btnSaveForm').on('click', function () {
            ...
            const pangkat = $('#field_pangkat').val().trim();
            if (!pangkat) {
                $('#field_pangkat').addClass('is-invalid');
                $('#err_pangkat').text('Pangkat wajib diisi.');
                return;
            }

            const golongan = $('#field_golongan').val().trim();
            if (!golongan) {
                $('#field_golongan').addClass('is-invalid');
                $('#err_golongan').text('Golongan wajib diisi.');
                return;
            }

            const ruang = $('#field_ruang').val().trim();
            if (!ruang) {
                $('#field_ruang').addClass('is-invalid');
                $('#err_ruang').text('Ruang wajib diisi.');
                return;
            }
            ...
            $.ajax({
                ...
                error: function (xhr) {
                    ...
                    if (errors.pangkat) {
                        $('#field_pangkat').addClass('is-invalid');
                        $('#err_pangkat').text(errors.pangkat[0]);
                    }

                    if (errors.golongan) {
                        $('#field_golongan').addClass('is-invalid');
                        $('#err_golongan').text(errors.golongan[0]);
                    }

                    if (errors.ruang) {
                        $('#field_ruang').addClass('is-invalid');
                        $('#err_ruang').text(errors.ruang[0]);
                    }

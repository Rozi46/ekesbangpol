							...
							<div class="col-md-12 bg_act_page_main">
								<div class="row">
									<div class="col-xl-12 col_act_page_main text-left">
										...
										@if($level_user['editusers'] == 'Yes')<button type="button" class="btn btn-primary" name="btn_save"><i class="fa fa-save"></i> Simpan Data</button>@endif
										@if($level_user['deleteusers'] == 'Yes')<button type="button" class="btn btn-danger" name="btn_del" onclick="DeleteData()"><i class="fa fa-trash"></i> Hapus Data</button>@endif
                ...
                <script type="text/javascript">
                    $(document).ready(function(){
                        ...
                        @if($level_user['editusers'] == 'No')
                            ...
                            $('button[name="btn_del"]').remove();
                        @endif
                        @if($detailadmin['id'] == session('admin_login_kesbangpol'))
                            $('form[name="form_data"] input').prop({disabled:true});
                            $('form[name="form_data"] select').prop({disabled:true});
                            $('button[name="btn_save"]').remove();
                            $('button[name="btn_del"]').remove();
